...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'lucky-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('lucky').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('lucky')[service]);
